      ...
      <div className="bg-white flex flex-col lg:flex-row w-full">
          ...
          <div className="lg:w-3/4 p-5 flex flex-col gap-5">
            <div className="">
              <h3 className="text-xl font-medium">
                Qual o valor do carro desejado?
              </h3>
              <div>
                {simulation.label} de
                <span className="font-medium text-lg text-indigo-600">{` R$${simulation.desiredValue}`}</span>
              </div>
              <input
                onChange={(e) =>
                  setSimulation({ ...simulation, desiredValue: e.target.value })
                }
                value={simulation.desiredValue}
                min={18000}
                max={500000}
                type="range"
              />
            ...
            <div>
              <h3 className="text-xl font-medium">
                Quando deseja ser contemplado?
              </h3>
              <div>
                No
                <span className="font-medium text-lg text-indigo-600">{` ${simulation.start}º mês`}</span>
              </div>
              <input
                onChange={(e) =>
                  setSimulation({ ...simulation, start: e.target.value })
                }
            ...
            <div>
              <h3 className="text-xl font-medium">
                Qual o melhor prazo para você?
              </h3>
              <div>Quanto maior o prazo, menor a parcela. Sem juros.</div>
              <div>
                <span className="font-medium text-lg text-indigo-600">{`${simulation.deadline} meses`}</span>
              </div>
              <input
                onChange={(e) =>
                  setSimulation({ ...simulation, deadline: e.target.value })
                }
        ...
        <div className="lg:w-1/4 rounded-md m-2  shadow h-auto flex flex-col gap-1">
          <div className="p-4 bg-gray-50 flex gap-1 justify-between">
            ...
            <select
              onChange={(e) =>
                setSimulation({ ...simulation, label: e.target.value })
              }
              value={simulation.label}
              className="px-2 py-1 border-blue-100 rounded ouline-blue-100 border-2"
            >
          ...
          <div className="p-4">
            ...
            <div>
              <h3 className="font-medium text-blue-600 text-base">
                Lance estimado para contemplação:
              </h3>
              <div>{2900.23}</div>
            </div>
            <div>
              <h3 className="font-medium text-blue-600 text-base">
                Valor da parcela antes do lance:
              </h3>
              <div>{880.89}</div>
            </div>
            <div>
              <h3 className="font-medium text-blue-600 text-base">
                Valor da parcela depois do lance:
              </h3>
              <div>{340.5}</div>
            </div>
          </div>
